//! Run `cargo check -p borrowing-deep` to see all errors.
...
fn ex06() {
    fn main() {
        ...
        nums.extend(std::iter::repeat(10).take(nums.len()))
    }
}
...
fn ex08() {
    ...
    let result = opt.unwrap_or_else(|| String::from("There"));
}

// ERROR 9: E0502 - NLL confusion
fn ex09() {
    let mut n = String::from("nll");
    let r = &n;
    println!("{}", r);
    n.push_str("x");
    println!("{:?}", n);
...
fn ex17() {
    ...
    make_ref(&mut t);
    fn make_ref(s: &mut String) -> &String {
        &s.push('d');
        s
    }
...
fn ex26() {
    let o: Option<String> = Some(String::from("x"));
    if let Some(s) = o {
        let _c = s;
    }
}
...
fn ex36() {
    use std::cell::RefCell;
    let o = RefCell::new(RefCell::new(5));
    let b = o.borrow();
    let i = b.borrow();
    let _im = o.borrow_mut();
}
